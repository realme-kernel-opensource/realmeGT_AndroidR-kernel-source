...
{
	uint64_t request_id = 0;
	uint32_t sof_event_status = CAM_REQ_MGR_SOF_EVENT_ERROR;
	struct cam_req_mgr_trigger_notify   notify;
	struct cam_ctx_request             *req;
	struct cam_isp_ctx_req             *req_isp;
	struct cam_context                 *ctx = ctx_isp->base;
	 ...
	 * the active req cnt is 2, and we always get the
	 * first req from active req list, in case shutter
	 * event isn't continuous.
	 */
	...
	if (((!req_isp->bubble_report) || (!req_isp->bubble_detected)) &&
		(req->request_id > ctx_isp->reported_req_id)) {
		request_id = req->request_id;
		ctx_isp->reported_req_id = request_id;
		CAM_DBG(CAM_ISP,

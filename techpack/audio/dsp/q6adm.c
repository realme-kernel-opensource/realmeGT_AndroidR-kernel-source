...
/* Yongzhi.Zhang@MULTIMEDIA.AUDIODRIVER.HAL, 2020/08/31, add for audio detect */
...
int adm_set_auddet_enable_param(int port_id, uint8_t val)
{
	...
	for (idx = 0; idx < MAX_COPPS_PER_PORT; idx++) {
		if (atomic_read(&this_adm.copp.id[port_idx][idx]) != RESET_COPP_ID) {
			...
			/* Yongzhi.Zhang@MULTIMEDIA.AUDIODRIVER.ADSP, 2020/12/19, add for not setting enable_flag when playback and recording */
...
int adm_get_all_mute_pp_param_from_port(int port_id)
{
	...
	for (idx = 0; idx < MAX_COPPS_PER_PORT; idx++) {
		if (atomic_read(&this_adm.copp.id[port_idx][idx]) != RESET_COPP_ID) {
			...
			/* Yongzhi.Zhang@MULTIMEDIA.AUDIODRIVER.ADSP, 2020/12/10, add for not reading results of playback and recording */
			if ((atomic_read(&this_adm.copp.app_type[port_idx][idx])) != 0x1113a) {
				continue;
			}

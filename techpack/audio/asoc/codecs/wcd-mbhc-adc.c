...
static int wcd_mbhc_adc_get_spl_hs_thres(struct wcd_mbhc *mbhc)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET, 2019/09/21, Add for log*/
...
static void wcd_micbias_disable(struct wcd_mbhc *mbhc)
{
	if (mbhc->micbias_enable) {
		#ifndef OPLUS_ARCH_EXTENDS
		/* Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET.861440, 2018/12/06, Modify for null pointer protect */
		mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic(
...
#ifdef OPLUS_ARCH_EXTENDS
/*Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET.2588226, 2019/12/17,
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	int hs_threshold, micbias_mv;
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET.1226693, 2017/03/03,
...
#ifdef OPLUS_ARCH_EXTENDS
/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET, 2018/12/06, Add for log*/
#undef pr_debug
#define pr_debug pr_info
#endif /* OPLUS_ARCH_EXTENDS */
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/*Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET.2573181, 2019/11/23,
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET ||
		...
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_DETECTION_DONE, 1);
		#ifdef OPLUS_ARCH_EXTENDS
		/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET, 2019/11/12,
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (output_mv > hs_threshold) {
			...
			wrk_complete = true;
			#ifdef OPLUS_ARCH_EXTENDS
			/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET.1033864, 2017/03/06, Add for HIGH_HPH loop detect.*/
			high_hph_count++;
				...
				if ((mbhc->current_plug !=
					...
					pr_debug("%s: cable is %s headset\n",
						...
						((spl_hs_count ==
							...
							"special ":""));
					#ifdef OPLUS_ARCH_EXTENDS
					/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET, 2019/11/23,
					 * Add for headset detect */
...
enable_supply:
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET.2635453, 2019/12/07,
...
exit:
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET.2784831, 2020/01/20, Add for log*/
...
#define pr_debug pr_info
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
		#ifndef OPLUS_ARCH_EXTENDS
		/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HEADSETDET.528838, 2020/11/13, Modify for headphone remove issue*/
		wcd_mbhc_hs_elec_irq(mbhc, WCD_MBHC_ELEC_HS_INS, false);
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_DETECTION_DONE, 1);
		wcd_mbhc_find_plug_and_report(mbhc, MBHC_PLUG_TYPE_HEADSET);
		WCD_MBHC_RSC_UNLOCK(mbhc);

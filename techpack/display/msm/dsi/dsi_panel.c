...
/* Kein.liuwq@PSW.MM.Display.LCD.Stability,2021-02-21 Add dly on off when going in and out of global HBM*/
...
*/
	...
	/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-11-04 add for global hbm */
	{
		...
		if ((!strcmp(panel->name,"samsung S6E3HC3 dsc cmd mode panel"))) {
			...
			/*yagnhanyue@RM.MM.Display.LCD.Params, 2020/12/03 add for 20627 panel*/
			if (bl_lvl > PANEL_MAX_NOMAL_BRIGHTNESS) {
			...
			/*kevin.liuwq@RM.MM.Display.LCD.Feature, 2021/01/25 add for global hbm*/
			if (bl_lvl > panel->bl_config.bl_normal_max_level) {
				enable_global_hbm_flags = 1;
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_HBM_ENTER_SWITCH);
				if (rc < 0)
...
// Pixelworks@MULTIMEDIA.DISPLAY, 2020/06/02, Iris5 Feature
	...
	/*yanghanyue@RM.MM.Display.LCD.Params, 2021/03/10 add to separate panel dc and color seed*/
...
#ifdef OPLUS_BUG_STABILITY
	/* ZhongLiuhe@MULTIMEDIA.DISPLAY.LCD.FEATURE, 2021/02/17, fixed that vsync is 15hz on AOD mode */
	if (oplus_adfr_is_support()) {

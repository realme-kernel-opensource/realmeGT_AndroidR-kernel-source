...
/*yanghanyue@RM.MM.Display.LCD.Params, 2020/12/31 remove old kevent interface*/
...
/*yanghanyue@RM.MM.Display.LCD.Params, 2021/03/10 add for 20627 panel dc config*/
...
/*#ifdef OPLUS_BUG_STABILITY*/
/* Kevin.liuwq@PSW.MM.Display.LCD.Stability, 2021/01/21, oplus cmd log */
			       ...
			       size_t len)
{
	...
	mutex_lock(&display->display_lock);
	/*yanghanyue@RM.MM.Display.LCD.Params, 2021/03/10 add to sync panel read and write*/
		...
		const char *buf, size_t count)
{
	...
	if (get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON) {
		...
		/*yanghanyue@RM.MM.Display.LCD.Params, 2021/03/10 add to ensure seed set succeed*/
		for (i = 0;i < 5;i++) {

...
/* Shengyang.Zhuo@BSP.CHG.Basic, 2020/11/20, sjc Add for usb temperature monitor */
		...
		charger_dev_set_input_current(g_oplus_chip->chgic_mtk.oplus_info->chg1_dev, 500000);
		/* LiYue@BSP.CHG.Basic, 2020/04/20, suspend mt6360 for swarp */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/* Kaili.Lu@BSP.CHG.Basic, 2020/10/19, Add for pd && swarp */
...
int oplus_mt6360_pd_setup_forswarp(void)
{
	...
	if(chip->pd_swarp){
		printk(KERN_ERR, "%s pd_swarp support\n", __func__);
		return 0;
	}

...
static int nu1619_set_silent_mode(bool is_silent)
{
	...
	if (is_silent) {
		chg_err("<~WPC~> set fan pwm pulse: %d\n", FAN_PWM_PULSE_IN_SILENT_MODE_2);
		nu1619_set_dock_fan_pwm_pulse(FAN_PWM_PULSE_IN_SILENT_MODE_2);
		chip->nu1619_chg_status.work_silent_mode = true;
	} else {
		chg_err("<~WPC~> set fan pwm pulse: %d\n", FAN_PWM_PULSE_IN_FASTCHG_MODE_2);
		nu1619_set_dock_fan_pwm_pulse(FAN_PWM_PULSE_IN_FASTCHG_MODE_2);
		chip->nu1619_chg_status.work_silent_mode = false;
	}
				      ...
				      size_t len, loff_t *lo)
{
	...
	if (pmw_pulse == FASTCHG_MODE) {
		...
		rc = nu1619_set_dock_fan_pwm_pulse(FAN_PWM_PULSE_IN_SILENT_MODE_2);
		if (rc == 0) {
			chip->quiet_mode_ack = false;

...
static irqreturn_t sdam_irq_handler(int irq, void *dev)
{
	struct cnss_pci_data *pci_priv = dev;

	cnss_schedule_recovery(&pci_priv->pci_dev->dev, CNSS_REASON_DEFAULT);
	pr_err("sdam_irq_handler\n");
	return IRQ_HANDLED;
}

static int cnss_register_sdam(struct cnss_pci_data *pci_priv)
{
	int rc;
	struct device *dev;
	struct cnss_sdam *sdam;
	struct cnss_plat_data *plat_priv = pci_priv->plat_priv;

	dev = &plat_priv->plat_dev->dev;

	sdam = devm_kcalloc(dev, 1, sizeof(struct cnss_sdam), GFP_KERNEL);
	if (!sdam)
		return -ENOMEM;

	rc = of_irq_get_byname(dev->of_node, "cnss-sdam19-irq");
	if (rc < 0) {
		pr_err("Failed to get irq for cnss-sdam19, err=%d\n", rc);
		return -EINVAL;
	}

	sdam->irq_name = devm_kstrdup(dev, "cnss-sdam19-irq", GFP_KERNEL);
	if (!sdam->irq_name)
		return -ENOMEM;
	sdam->irq = rc;
	...
	sdam->nvmem = devm_nvmem_device_get(dev, "cnss-sdam19");
	...
	rc = devm_request_threaded_irq(dev, sdam->irq,
			NULL, sdam_irq_handler,
			IRQF_ONESHOT, sdam->irq_name, pci_priv);
	if (rc < 0) {
		pr_err("Failed to request IRQ(%s), rc=%d\n",
			sdam->irq_name, rc);
		return rc;
	}
	...
	case QCA6490_DEVICE_ID:
		...
		if (ret)
			cnss_pr_err("Failed to suspend PCI link, err = %d\n",
				    ret);
		cnss_power_off_device(plat_priv);
		ret = cnss_register_sdam(pci_priv);
		if (ret)
		    cnss_pr_err("CNSS sdam init failed %d\n", ret);
		break;
	default:

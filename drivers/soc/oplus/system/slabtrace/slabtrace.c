...
static void* oplus_fixup_red_left(struct kmem_cache *s, void *p)
{
    ...
    if (unlikely(s->flags & SLAB_DEBUG_FLAGS) && s->flags & SLAB_RED_ZONE)
	    p += s->red_left_pad;
#endif
	return p;
} 

static void oplus_get_map(struct kmem_cache *s, struct page *page, unsigned long *map)
{
	void *p;
	void *addr = page_address(page);

	for (p = page->freelist; p; p = oplus_get_freepointer(s, p))
		set_bit(slab_index(p, s, addr), map);
}

static inline void *oplus_get_freepointer(struct kmem_cache *s, void *object)
{
	void * ptr = (void *)*(unsigned long *)object;
	unsigned long ptr_addr = (unsigned long)object;
...
//sort by pid
	 ...
	 */
	if (t->count >= t->max &&
		!oplus_alloc_loc_track(t, 2 * t->max, __GFP_HIGH | __GFP_ATOMIC))
		return 0;
...
//sort by pid
	...
	if (!map || !oplus_alloc_loc_track(&t, PAGE_SIZE / sizeof(struct location),
				     GFP_KERNEL)) {
		kfree(map);
